...
| AI Chat | POST | `/api/chat` | user/admin | Hỏi–đáp AI (LLM + tools theo quyền người hỏi). Body: `{"message"(≤4000), "session_id"(**UUID**, tùy chọn), "include_latest_watcher_context"(mặc định true), "max_context_chars", "stream"(mặc định false)}`. Mặc định trả JSON `{status, session_id, reply, model, provider, execution_context_used}` (lỗi: `{status:"error", error_code, message, retryable}`). Đặt **`"stream": true`** → trả **SSE** (`text/event-stream`), mỗi event là JSON có `type` = `meta`/`thinking`/`delta`/`tool_call`/`tool_result`/`done`/`error`, kết thúc bằng `data: [DONE]`. **Hội thoại lưu theo user.** `session_id` phải là **UUID**: UUID đã có (của mình)→nối tiếp; UUID chưa có→phiên mới với id đó; bỏ trống→phiên mới; text như `"demo"`→**422**. |
