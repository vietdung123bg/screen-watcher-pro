...
9. Tab **📓 Jupyter** (admin): khởi động **Jupyter server** cho `notebooks/chatbox.ipynb` (client notebook gọi REST API) ngay trong app. Bấm **▶ Start Jupyter** → app tự lấy URL kèm token từ log, rồi bấm **📓 Open notebook** để mở đúng notebook trên trình duyệt. **■ Stop** để tắt (tự tắt khi thoát app). Cần cài Jupyter (`pip install notebook`); nếu thiếu, tab báo rõ.
